...
df = pd.read_csv(r'/home/aayush/Downloads/training.csv')
s = df['image_name']

images = []
for i in range(len(s)):
    loc = r'/home/aayush/Downloads/images_train/'+s[i]
    img1 = Image.open(loc)
    nx , ny = img1.size
...
df = pd.read_csv(r'/home/aayush/Downloads/test.csv')
s = df['image_name']

images = []
for i in range(len(s)):
    loc = r'/home/aayush/Downloads/images_test/'+s[i]
    img1 = Image.open(loc)
    nx , ny = img1.size
    img = np.asarray(img1.resize((int(nx*1),int(ny*1)),Image.BICUBIC))
    images.append(img)
...
pred = model.predict(X_t, batch_size = 32)
df['x1'] = pred[:, 0]*640
df['x2'] = pred[:, 1]*640
df['y1'] = pred[:, 2]*480
df['y2'] = pred[:, 3]*480
